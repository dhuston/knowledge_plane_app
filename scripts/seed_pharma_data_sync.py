...
def main():
    ...
    try:
        # Get database engine
        database_url = "postgresql://postgres:password@db/knowledgeplan_dev"
        ...
        try:
            ...
            # Get the tenant
            tenant_query = f"SELECT * FROM tenants WHERE id = '{TENANT_ID}'"
            ...
            if not tenant:
                ...
                VALUES ('{TENANT_ID}', 'Pharma AI Demo', 'pharmademo.biosphere.ai', true)
                """
                db.execute(tenant_sql)
                db.commit()
            ...
            db.execute(f"DELETE FROM edges WHERE tenant_id = '{TENANT_ID}'")
            db.execute(f"DELETE FROM nodes WHERE tenant_id = '{TENANT_ID}'")
            db.execute(f"DELETE FROM knowledge_assets WHERE tenant_id = '{TENANT_ID}'")
            db.execute(f"DELETE FROM project_participants WHERE project_id IN (SELECT id FROM projects WHERE tenant_id = '{TENANT_ID}')")
            db.execute(f"DELETE FROM projects WHERE tenant_id = '{TENANT_ID}'")
            db.execute(f"DELETE FROM goals WHERE tenant_id = '{TENANT_ID}'")
            db.execute(f"DELETE FROM users WHERE tenant_id = '{TENANT_ID}' AND id != '{ADMIN_USER_ID}'")
            db.execute(f"DELETE FROM teams WHERE tenant_id = '{TENANT_ID}'")
            db.execute(f"DELETE FROM departments WHERE tenant_id = '{TENANT_ID}'")
            db.commit()
            ...
            for user in users:
                manager_id = f"'{user['manager_id']}'" if user['manager_id'] else 'NULL'
                insert_sql = f"""
            ...
            for goal in goals:
                parent_id = f"'{goal['parent_id']}'" if 'parent_id' in goal and goal['parent_id'] else 'NULL'
                due_date = f"'{goal['due_date']}'" if 'due_date' in goal and goal['due_date'] else 'NULL'
                insert_sql = f"""
                INSERT INTO goals (
                    id, tenant_id, title, description, type, 
                    status, progress, due_date, parent_id
                )
                VALUES (
                    '{goal["id"]}',
                    '{goal["tenant_id"]}',
            ...
            for project in projects:
                goal_id = f"'{project['goal_id']}'" if project['goal_id'] else 'NULL'
                insert_sql = f"""
                INSERT INTO projects (
                    id, tenant_id, name, description, status, owning_team_id, goal_id
                )
                VALUES (
                    '{project["id"]}',
                    '{project["tenant_id"]}',
            ...
            for i in range(0, len(knowledge_assets), batch_size):
                ...
                for asset in batch:
                    ...
                    INSERT INTO knowledge_assets (
                        id, tenant_id, title, type, source, content, 
            ...
            for i in range(0, len(nodes), batch_size):
                ...
                for node in batch:
                    props_json = str(node["props"]).replace("'", '"')
                    insert_sql = f"""
            ...
            for i in range(0, len(edges), batch_size):
                ...
                for edge in batch:
                    props_json = "{}" if not edge["props"] else str(edge["props"]).replace("'", '"')
                    insert_sql = f"""
