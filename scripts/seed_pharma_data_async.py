#!/usr/bin/env python3
"""
Database seeding script for Pharma AI Demo tenant using asyncpg
...
def generate_knowledge_assets(user_ids, project_ids):
    ...
    asset_types = [
        ("NOTE", 0.3),          # 30% are notes
        ("DOCUMENT", 0.3),      # 30% are documents
        ("REPORT", 0.15),       # 15% are reports
        ("MEETING", 0.1),       # 10% are meeting notes
        ("PRESENTATION", 0.1),  # 10% are presentations
        ("SUBMISSION", 0.05)    # 5% are submissions
    ]
    ...
    while len(asset_type_distribution) < 500:
        asset_type_distribution.append("NOTE")
...
async def main():
    ...
    try:
        ...
        try:
            ...
            tenant = await conn.fetchrow(f"SELECT * FROM tenants WHERE id = '{TENANT_ID}'")
            if not tenant:
                # Create a tenant if it doesn't exist
                await conn.execute("""
                INSERT INTO tenants (id, name, domain, is_active)
                VALUES ($1, $2, $3, $4)
                """, TENANT_ID, "Pharma AI Demo", "pharmademo.biosphere.ai", True)
                print(f"Created tenant: Pharma AI Demo")
            else:
                print(f"Using existing tenant: {tenant['name']}")
            
            # Clear existing data for this tenant
            print("Clearing existing tenant data...")
            await conn.execute(f"DELETE FROM edges WHERE tenant_id = '{TENANT_ID}'")
            await conn.execute(f"DELETE FROM nodes WHERE tenant_id = '{TENANT_ID}'")
            await conn.execute(f"DELETE FROM knowledge_assets WHERE tenant_id = '{TENANT_ID}'")
            await conn.execute(f"DELETE FROM project_participants WHERE project_id IN (SELECT id FROM projects WHERE tenant_id = '{TENANT_ID}')")
            await conn.execute(f"DELETE FROM projects WHERE tenant_id = '{TENANT_ID}'")
            await conn.execute(f"DELETE FROM goals WHERE tenant_id = '{TENANT_ID}'")
            await conn.execute(f"DELETE FROM users WHERE tenant_id = '{TENANT_ID}' AND id != '{ADMIN_USER_ID}'")
            await conn.execute(f"DELETE FROM teams WHERE tenant_id = '{TENANT_ID}'")
            await conn.execute(f"DELETE FROM departments WHERE tenant_id = '{TENANT_ID}'")
            ...
            for dept in departments:
                ...
                VALUES ($1, $2, $3, $4)
                """, dept["id"], TENANT_ID, dept["name"], dept["description"])
            ...
            for team in teams:
                ...
                VALUES ($1, $2, $3, $4)
                """, team["id"], TENANT_ID, team["name"], team["description"])
            ...
            for user in users:
                await conn.execute("""
                INSERT INTO users (id, tenant_id, name, email, title, team_id, manager_id)
                VALUES ($1, $2, $3, $4, $5, $6, $7)
                """, user["id"], user["tenant_id"], user["name"], user["email"], 
                    user["title"], user["team_id"], user["manager_id"])
            ...
            for goal in goals:
                ...
                await conn.execute("""
                INSERT INTO goals (id, tenant_id, title, description, type, status, progress, due_date, parent_id)
                VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
                """, goal["id"], goal["tenant_id"], goal["title"], goal["description"], 
                    goal["type"], goal["status"], goal["progress"], due_date, parent_id)
            ...
            for project in projects:
                await conn.execute("""
                INSERT INTO projects (id, tenant_id, name, description, status, owning_team_id, goal_id)
                VALUES ($1, $2, $3, $4, $5, $6, $7)
                """, project["id"], project["tenant_id"], project["name"], project["description"], 
                    project["status"], project["owning_team_id"], project["goal_id"])
            ...
            for i in range(0, len(knowledge_assets), batch_size):
                ...
                for asset in batch:
                    values_list.append((
                        asset["id"], asset["tenant_id"], asset["title"], asset["type"], 
                        asset["source"], asset["content"], asset["project_id"], asset["created_by_user_id"]
                    ))
                
                # Use executemany for batch insert
                await conn.executemany("""
                INSERT INTO knowledge_assets (id, tenant_id, title, type, source, content, project_id, created_by_user_id)
            ...
            for i in range(0, len(nodes), batch_size):
                ...
                for node in batch:
                    ...
                    values_list.append((
                        node["id"], node["tenant_id"], node["type"], 
                        props_json, node["x"], node["y"]
                    ))
            ...
            for i in range(0, len(edges), batch_size):
                ...
                values_list = []
                for edge in batch:
                    props_json = json.dumps(edge.get("props", {}))
                    values_list.append((
                        edge["id"], edge["tenant_id"], edge["src"], 
                        edge["dst"], edge["label"], props_json
                    ))
