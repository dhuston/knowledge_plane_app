        ...
        goals = [
            {
                ...
                "type": "ENTERPRISE",
                "priority": "HIGH",
                "target_date": (now.replace(year=now.year+1)).isoformat(),
            ...
            },
            {
                ...
                "type": "DEPARTMENT",
                "priority": "HIGH",
                "target_date": (now.replace(month=(now.month+2) % 12 or 12)).isoformat(),
            ...
            },
            {
                ...
                "type": "DEPARTMENT",
                "priority": "MEDIUM",
                "target_date": (now.replace(month=(now.month+3) % 12 or 12)).isoformat(),
            ...
            },
            {
                ...
                "type": "TEAM",
                "priority": "HIGH",
                "target_date": (now.replace(month=(now.month+1) % 12 or 12)).isoformat(),
        ...
        for goal in goals:
            ...
            if not goal_id:
                ...
                for team_name in goal["teams"]:
                    ...
                    # Create edge from team to goal - CORRECTED COLUMN NAMES: src, dst, label
                    edge_id = str(uuid.uuid4())
                    edge_query = """
                    INSERT INTO edges (id, tenant_id, src, dst, label, props)
                    VALUES (:edge_id, :tenant_id, :src, :dst, :label, :props)
        ...
        for project in projects:
            ...
            if not project_id:
                ...
                for team_name in project["teams"]:
                    ...
                    if team_node_id:
                        # Create edge from team to project - FIXED COLUMN NAMES
                        edge_id = str(uuid.uuid4())
                        edge_query = """
                        INSERT INTO edges (id, tenant_id, src, dst, label, props)
                        VALUES (:edge_id, :tenant_id, :src, :dst, :label, :props)
                ...
                for goal_name in project["goals"]:
                    ...
                    if goal_id:
                        ...
                        if goal_node_id:
                            # Create edge from project to goal - FIXED COLUMN NAMES
                            edge_id = str(uuid.uuid4())
                            edge_query = """
                            INSERT INTO edges (id, tenant_id, src, dst, label, props)
                            VALUES (:edge_id, :tenant_id, :src, :dst, :label, :props)
